...
if($OJ_MEMCACHE)
       ...
       if ($row['contest_id']>0) {
        ...
        if(isset($_SESSION['http_judge'])) {
       //           $view_status[$i][3].="<form method=post action=admin/problem_judge.php><input type=hidden name=sid value='".$row['solution_id']."'>";
                  $view_status[$i][3].="<select class='btn input-mini' length=4 name=result>";
                  $view_status[$i][3].="<option value=0>手动</a>";
                  $view_status[$i][3].="<option value=4>$MSG_AC</a>";
                  $view_status[$i][3].="<option value=6>$MSG_WA</a>";
                  $view_status[$i][3].="</select>";
                  $view_status[$i][3].="<input title='解释'  class='btn input-small'  type='text' name='explain'>";
                  $view_status[$i][3].="<input class='btn'  type=submit name=manual value='确定'>";
